...
*/
async function uploadAllResultsFromPubMedToDB() {
  my_keywords = ["covid", "covid+symptoms", "covid+vaccine", "molecular+epidemiology", "clinical"];
  /* TODO: Make all of these keywords execute and run
  *   */
  for (k in my_keywords) {
    new MyXMLHTTPRequest(new PubMedURLs().main_url).uploadSearchResultsToFirestore(k, "covid_pubmed_search");
    new MyXMLHTTPRequest(new PubMedURLs().main_url).getStatisticsAboutKeyword(k);
    break;
  }
}

//Uploads all tweets doing with covid avalibility in florida counties and uploads it to firebase functions as a function
exports.getPubMedData = functions.runWith({timeoutSeconds: 539}).pubsub.schedule('every 5 days').onRun( async(context) => {
  await uploadAllResultsFromPubMedToDB().then(response => {
